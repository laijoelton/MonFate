...
export default function AdminPage() {
  const [section, setSection] = useState<AdminPage>("overview");
  ...
  const sectionTitle: Record<AdminPage, string> = {
    overview: "Overview",
    map: "Live Map & Ops",
    reports: "Bus Reports",
  ...
  return (
    <div className="monfate-cockpit flex">
      <AdminNav
        activePage={section}
        onNavigate={setSection}
        pendingReportsCount={reports.length}
        activeIssuesCount={activeObstacles.length}
      />

      <div className="min-h-screen flex-1 px-4 py-8 pb-24 sm:px-8 md:pb-8">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Card title="Fleet accessibility" icon={<ShieldCheck className="h-4 w-4" />}>
                  <div className="flex items-center gap-4">
                    ...
                    <p className="text-sm text-slate-400">
                      <span className="tabular font-semibold text-slate-100">{accessiblePct}%</span> of the fleet has
                      a working ramp right now.
                    </p>
                  </div>
                </Card>
                <Card title="Wheelchair space" icon={<ShieldCheck className="h-4 w-4" />}>
                  <div className="flex items-center gap-4">
                    ...
                    <p className="text-sm text-slate-400">
                      <span className="tabular font-semibold text-slate-100">{spacePct}%</span> of the fleet
                      currently has a free wheelchair space.
                    </p>
                  </div>
                </Card>
              </div>
          ...
          {section === "accessibility" && (
            <>
              <Card
                ...
                right={
                  <StatusPill tone={activeObstacles.length > 0 ? "down" : "ok"}>
                    {activeObstacles.length} active
                  </StatusPill>
                }
              >
                    ...
                    >
                      <div>
                        <p className="text-sm font-semibold capitalize">
                          {obstacle.obstacle_type.replace(/_/g, " ")}
                        </p>
              ...
              <Card
                ...
                right={
                  <StatusPill tone={tripRequests.length > 0 ? "accent" : "idle"}>
                    {tripRequests.length} total
                  </StatusPill>
                }
              >
                      ...
                      {ranked.map(([stopId, count]) => {
                        ...
                        return (
                          <div key={stopId} className="flex items-center gap-3">
                            ...
                            <span className="w-6 flex-shrink-0 text-right text-xs tabular text-slate-400">
                              {count}
                            </span>
                          </div>
                        );
