...
export function useUserProfile() {
  ...
  useEffect(() => {
    // Deferred so the stored value lands in a callback rather than
    // synchronously in the effect body, which cascades a render.
    const id = window.setTimeout(() => setNeeds(readStoredNeeds()), 0);
